...
class Visualizer:
    ...
    def vis_roc_curve(self, output):
        ...
        self.plot_roc_curve_new(fpr, tpr)
    ...
    def plot_roc_curve(fpr, tpr, name='model', fig=None):
        if fig is None:
            fig = plt.figure()
            plt.plot(np.arange(0, 1, 0.01), np.arange(0, 1, 0.01), 'r', linestyle='--', label='Random guess')
        roc_auc = metrics.auc(fpr, tpr)
        plt.plot(fpr, tpr, color=(random.uniform(0, 1), random.uniform(0, 1), random.uniform(0, 1)),
                     label='ROC curve with {} - AUC = {}]'.format(name, roc_auc))
    ...
    @staticmethod
    def plot_roc_curve_new(fpr, tpr, name='model', fig=None):
    ...
    def vis_det_curve(self, output):
        ...
        self.plot_det_curve(fpr, fnr)
        filepath = os.path.join(output, "det.jpg")
        plt.savefig(filepath)
        return fpr, fnr, pos, neg

    @staticmethod
    def plot_det_curve(fpr, fnr, name='model', fig=None):
        if fig is None:
            fig = plt.figure()
            #plt.plot(np.arange(0, 1, 0.01), np.arange(0, 1, 0.01), 'r', linestyle='--', label='Random guess')
        plt.semilogx(fpr, fnr, color=(random.uniform(0, 1), random.uniform(0, 1), random.uniform(0, 1)),
                 label='DET curve with {}'.format(name))
