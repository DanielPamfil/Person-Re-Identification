...
def reid_score(q_features, q_pids, q_cameras, g_features, g_pids, g_cameras):
    dist = tensor2numpy(pairwise_distance(q_features, g_features))
    q_pids = tensor2numpy(q_pids)
    q_cameras = tensor2numpy(q_cameras)
    g_pids = tensor2numpy(g_pids)
    g_cameras = tensor2numpy(g_cameras)
    ...
    for i in range(len(q_pids)):
        ...
        ql, qc, gl, gc = q_pids[i], q_cameras[i], g_pids, g_cameras
    ...
    cmc = (cmc.float()) / len(q_pids)  # average cmc
    ap = ap / len(q_pids)
    return ap, cmc
